...
| `openapi` | OpenAPI 文件檢視 | 開發者工具 | Scalar API Reference（本地函式庫）渲染 OpenAPI/Swagger 文件，於新分頁開啟獨立頁面（Blob URL）；支援網址（可選 Scalar CORS Proxy）、貼上 JSON/YAML、拖曳上傳檔案；版面配置可選現代/經典（類似 Swagger UI）；下載 HTML 時可選 Scalar 或 Swagger UI 原始 JS 引擎渲染（皆改走 jsDelivr CDN，需連網開啟） | ✓ | ✓ `#oaRenderBtn` |
